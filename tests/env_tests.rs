...
"#;
    ...
    assert_eq!(env_manager.get("NODE_ENV"), Some(&"development".to_string()));
    assert_eq!(env_manager.get("API_KEY"), Some(&"secret123".to_string()));
    assert_eq!(env_manager.get("DATABASE_URL"), Some(&"postgresql://user:pass@localhost/db".to_string()));
...
fn test_env_manager_set_and_get() {
    ...
    assert_eq!(env_manager.get("CUSTOM_VAR"), Some(&"custom_value".to_string()));
    
    env_manager.set("CUSTOM_VAR".to_string(), "updated_value".to_string());
    assert_eq!(env_manager.get("CUSTOM_VAR"), Some(&"updated_value".to_string()));
}
...
"#;
    ...
    assert_eq!(env_manager.get("VAR_WITH_EQUALS"), Some(&"value=with=equals".to_string()));
    assert_eq!(env_manager.get("VAR_WITH_SPACES"), Some(&"value with spaces".to_string()));
    assert_eq!(env_manager.get("VAR_WITH_QUOTES"), Some(&"single quoted".to_string()));
    assert_eq!(env_manager.get("VAR_WITH_DOUBLE_QUOTES"), Some(&"double quoted".to_string()));
